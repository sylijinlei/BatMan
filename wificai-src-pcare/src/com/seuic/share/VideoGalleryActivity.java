...
import com.beelinker.media.player;
...
public class VideoGalleryActivity extends Activity {
	...
	protected void onStop() {
		Log.e("VideoGallery", "in onStop ...");
		super.onStop();
		if (isShared && !isExit) {
			isShared = false;
		}else if (isBackgroud) {
			Log.e("VideoGallery", "set isBackground false ...");
			isBackgroud = false;
		}else if (!isShared && !isExit && !AppActivityClose.isExit) {
			Log.e("VideoGallery", "exit application");
			isExit = true;
	...
	protected void onDestroy() {
		Log.e("VideoGallery", "in onDestroy ...");
		for (int i = 0; i < 2; i++) {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		Log.e("VideoGallery", "in onCreate ...");
		mContext = this;
	    requestWindowFeature(Window.FEATURE_NO_TITLE);
		getWindow().setFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON, WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON);
		wificarNewLayoutParams = WificarNewLayoutParams.getWificarNewLayoutParams(VideoGalleryActivity.this);
		if (parser == null) {
	...
	public void Delete_video() {
		...
		if (VideoImage.imageAdapterV != null) {
			VideoImage.imageAdapterV.removePhoto(positionV);		
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (convertView == null) {
				...
				if (wificarNewLayoutParams.screenSize > 5.8) {
					convertView = inflater1.inflate(R.layout.video_hplay_item, null);
					holderView = new HolderView();
					holderView.imgv = (ImageView)convertView.findViewById(R.id.imageView_video_play1);
					holderView.playBtn = (ImageView)convertView.findViewById(R.id.video_play_button1);
					convertView.setTag(holderView);
					Log.e("VideoGalleryActivity", ">5.8");
				} else {
					convertView = inflater1.inflate(R.layout.video_play_item, null);
					holderView = new HolderView();
					holderView.imgv = (ImageView)convertView.findViewById(R.id.imageView_video_play2);
					holderView.playBtn = (ImageView)convertView.findViewById(R.id.video_play_button2);
					convertView.setTag(holderView);
					Log.e("VideoGalleryActivity", "<5.8");
				}
			...
			if (bitmap[0] == null) {
				bitmap[1] = getVideoThumbnail(bitmap[0], video_path.get(position).toString(),640,480,MediaStore.Images.Thumbnails.MINI_KIND);
	...
	public OnClickListener videoPlayListent = new OnClickListener() {
		public void onClick(View v) {
			...
			if(file.exists()){
				Intent intent = new Intent(VideoGalleryActivity.this, player.class);
