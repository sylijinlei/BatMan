...
public class PhotoImage extends Activity{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		path = AppInforToCustom.getAppInforToCustomInstance().getCameraPicturePath();
		photo = getInSDPhoto();
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		if (keyCode == KeyEvent.KEYCODE_BACK) {
			if (!ShareActivity.isBack) {
				...
				ShareActivity.instance.ShowInfo(R.string.click_again_to_exit_the_program,
						Toast.LENGTH_SHORT);
				BackTimer = new Timer();
    ...
    class AsyncTaskLoadPhoto extends AsyncTask<Object, Bitmap, Object> {
		...
		protected Object doInBackground(Object... params) {
			...
			for(int i = 0; i < size; i++){
				...
				bitmapP =  getImageThumbnail(bitmapP, photo_lis.get(i).toString(), 160, 120);
				if (bitmapP != null) {
					publishProgress(bitmapP);
				}else{
					Bitmap bitmapP2 = BitmapFactory.decodeFile( photo_lis.get(i).toString()); 
		            if(bitmapP2 !=null){
		            	bitmapP = Bitmap.createScaledBitmap(bitmapP2,160,120,true);
		            	publishProgress(bitmapP);
		            	bitmapP2.recycle();
		            }
				}
